    ...
    public sealed class HTTPRequest
    {
        ...
        private void CloseSink(HTTPSession ss)
        {
            ...
            lock (TagQueue)
            {
                ...
                if (TagQueue.Count > 0)
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Information, "Switching Pipeline Modes [" + ss.GetHashCode().ToString() + "]");
                    _PIPELINE = false;
                    if (!ReceivedFirstResponse)
                    {
                ...
                if (!ReceivedFirstResponse)
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "Server[" + erraddr + "] closed socket without answering");
                    err = true;
                }
        ...
        private void CreateFailedSink(HTTPSession ss)
        {
            lock (TagQueue)
            {
                ...
                {
                    StateData sd = (StateData) TagQueue.Dequeue();
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "Connection Attempt to [" + sd.Dest.ToString() + "] Refused/Failed");
