                    ...
                    if (sessions.ContainsKey(addr) == false)
                    {
                        try
                        {
                            if (addr.AddressFamily == AddressFamily.InterNetwork)
                            {
                                ...
                                if (!Utils.IsMono()) session.Client.Bind(new IPEndPoint(addr, 1900));
                                else session.Client.Bind(new IPEndPoint(IPAddress.Any, 1900));
                                session.EnableBroadcast = true;
                                session.JoinMulticastGroup(Utils.UpnpMulticastV4Addr, addr);
                                try
                                {
                                    if (!Utils.IsMono()) session.Client.IOControl(SIO_UDP_CONNRESET, inValue, outValue);
                                }
                                catch (SocketException ex)
                                {
                                    EventLogger.Log(ex);
                                ...
                                try
                                {
                                    if (!Utils.IsMono()) usession.Client.IOControl(SIO_UDP_CONNRESET, inValue, outValue);
                                }
                                catch (SocketException ex)
                                {
                                    EventLogger.Log(ex);
                            ...
                            if (addr.AddressFamily == AddressFamily.InterNetworkV6)
                            {
                                ...
                                if (!Utils.IsMono()) session.Client.Bind(new IPEndPoint(addr, 1900));
                                else session.Client.Bind(new IPEndPoint(IPAddress.IPv6Any, 1900));
                                session.EnableBroadcast = true;
                                if (addr.IsIPv6LinkLocal) session.JoinMulticastGroup((int) addr.ScopeId, Utils.UpnpMulticastV6Addr2);
                                else session.JoinMulticastGroup((int) addr.ScopeId, Utils.UpnpMulticastV6Addr1);
                                try
                                {
                                    if (!Utils.IsMono()) session.Client.IOControl(SIO_UDP_CONNRESET, inValue, outValue);
                                }
