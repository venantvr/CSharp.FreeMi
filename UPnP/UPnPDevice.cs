...
namespace OpenSource.UPnP
{
    ...
    public sealed class UPnPDevice
    {
        ...
        {
            get { return _icon; }
            set { if (this.ControlPointOnly == false) _icon = value; }
        }

        public System.Drawing.Image Icon2
        {
            get { return _icon2; }
            set { if (this.ControlPointOnly == false) _icon2 = value; }
        }
        ...
        {
            get { return _favicon; }
            set { if (this.ControlPointOnly == false) _favicon = value; }
        }
        ...
        private void NewDeviceInterface(NetworkInfo sender, IPAddress ip)
        {
            ...
            catch (SocketException ex)
            {
                ...
                EventLogger.Log(ex);
                EventLogger.Log(ex, "UPnPDevice: " + this.FriendlyName + " @" + ip.ToString());
        ...
        public String StandardDeviceType
        {
            ...
            set { DeviceURN = string.Format("urn:schemas-upnp-org:device:{0}:{1}", value, Version); }
        }
        ...
        {
            get { return (Arch_Major.ToString() + "." + Arch_Minor.ToString()); }
            set
            {
                DText p = new DText();
                p.ATTRMARK = ".";
        ...
        internal static String highlightError(String original, int line, int charPosition)
        {
            int i, j, k;
                                    ...
                                    switch (XMLDoc.NodeType)
                                    {
                                        case XmlNodeType.Element:
                                            if (XMLDoc.LocalName == "icon")
                                            {
                                                ...
                                                if (XMLDoc.NodeType == XmlNodeType.EndElement && XMLDoc.LocalName == "iconList")
                                                {
                                                    finishedIconList = true;
                                                }
                                            }
                                            break;
                                        case XmlNodeType.EndElement:
                                            if (XMLDoc.LocalName == "iconList")
                                            {
                                                finishedIconList = true;
                                            }
                                            break;
        ...
        private void HandleIcon(HTTPRequestor sender, bool success, object tag, string url, byte[] data)
        {
