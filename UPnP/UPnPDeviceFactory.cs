...
namespace OpenSource.UPnP
{
    ...
    public sealed class UPnPDeviceFactory
    {
        ...
        {
            EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Information, "Creating Device: " + usn);
        ...
        private void httprequestor_OnRequestCompleted(HTTPRequestor sender, bool success, object tag, string url, byte[] data)
        {
            try
            {
                if (url == null)
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "HTTP: Url is empty");
                ...
                if (!success)
                {
                    ...
                    try
                    {
                        EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "HTTP: Could not connect to target: " + url);
                        //checkpoint = 1;
                ...
                if (data == null)
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "HTTP: Data is empty");
                ...
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, string.Format("Unique ID mismatch between SSDP packet and device description: {0} / {1} ", TempDevice.UniqueDeviceName, expected_usn));
