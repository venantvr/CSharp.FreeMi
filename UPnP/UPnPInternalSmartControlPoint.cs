...
namespace OpenSource.UPnP
{
    ...
    internal sealed class UPnPInternalSmartControlPoint
    {
        ...
        private void DeviceFactoryCreationSink(UPnPDeviceFactory sender, UPnPDevice device, Uri locationURL)
        {
            ...
            {
                EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "UPnPDevice[" + device.FriendlyName + "]@" + device.LocationURL + " advertised UDN[" + device.UniqueDeviceName + "] in xml but not in SSDP");
                return;
            }
            ...
            lock (deviceTableLock)
            {
                ...
                {
                    EventLogger.Log(this, System.Diagnostics.EventLogEntryType.Error, "Unexpected UPnP Device Creation: " + device.FriendlyName + "@" + device.LocationURL);
                    return;
                }
